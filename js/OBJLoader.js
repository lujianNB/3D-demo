...
THREE.OBJLoader = (function () {
	...
	OBJLoader.prototype = {
		...
		load: function (url, onLoad, onProgress, onError, search) {
			...
			loader.load(url, function (text) {
				...
				onLoad(scope.parse(text, search));
						...
						case 'v':
							...
							if (data.length === 8) {
								if (search) {
									let flag = true
									search.forEach(item => {
										if (data[1] <= item.StackEnd * 1000 && data[1] > item.StackStart * 1000) {
											flag = false
											if (item.TYPE === 2) {
												state.colors.push(
													parseFloat(1),
